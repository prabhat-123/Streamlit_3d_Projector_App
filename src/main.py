...
def display_props():
    # header
    st.markdown(
        "####  Embeddings explorer with different Reduction Schemes and Search Functionality"
    )
    # feature image
...
def display_reductions():
    ...
    reductions_type = st.sidebar.selectbox(
        "Select Dim. Reduction", options, format_func=lambda x: reductions[x]
    )
...
def plot_with_opacity(df, search):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            opacity=1,
            marker=dict(
                color="LightSkyBlue", size=30, line=dict(color="MediumPurple", width=2)
            ),
            name="Opacity 1.0",
        )
    )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            opacity=0.2,
            marker=dict(
                color="LightSkyBlue", size=5, line=dict(color="MediumPurple", width=2)
            ),
            name="Opacity 0.2",
        )
    )
...
def plot_with_3d_opacity(df, search):
    ...
    fig.add_trace(
        go.Scatter3d(
            ...
            x=df.loc[df["index"].isin(search), "pc1"],
            y=df.loc[df["index"].isin(search), "pc2"],
            z=df.loc[df["index"].isin(search), "pc3"],
            opacity=1,
            marker=dict(
                color="red", size=30, line=dict(color="red", width=2)
            ),
            name="Opacity 1.0",
        )
    )
    ...
    fig.add_trace(
        go.Scatter3d(
            ...
            opacity=0.2,
            marker=dict(
                color="pink", size=5, line=dict(color="pink", width=2)
            ),
            name="Opacity 0.6",
        )
    )
...
def plot_3d_updated(df, labels, need_labels, index, search=None):
    ...
    if not need_labels:
        labels = None
    if search:
        fig = plot_with_3d_opacity(df, search)
    else:
        fig = px.scatter_3d(
			df,
			x="pc1",
        ...
        if button:
            if dim == "2-D":
                ...
                if search_for:
                    search_idx = search_for
                    plot_for_3D(
                        pca_df, labels, need_labels, indexes, need_animation, search_idx
                    )
                else:
